...
$(function()
{
	...
	{
		if (event.keyCode == 13)			//按下回车时，发送，并且阻止按钮点击跳转，并把用户输入清空
		{
			send($(this).val());
			$(this).val('');
...
function start_blur()				//not important
{
	try
	{
...
function on_focus()					//not important
{
	try
	{
		clearTimeout(window.blur_timer);
...
function init_chat()
{
	...
	$.get('start.php?t='+Math.random(),{},function(_id)		//跳转start.php，将id和ip地址信息放到数据库
	{
		id = parseInt(_id);
		if (id)
		{
...
function load_event(id)
{
	...
	if (user.send.length>0)
		user.send_content = user.send.shift();			//把push的发送数据的第一条shift出来放到content里
	else
		user.send_content = '';
